...
class DatabaseSeeder extends Seeder
{
            ...
            if ($this->command->confirm('Creation Roles pour user, par défaut  admin et user? [y|N]', true)) {
                ...
                foreach($roles_array as $role) {
                    ...
                    if( $role->name == 'Admin' ) {
                        ...
                        //$this->command->info(Pemission::all());
                    } else {
                ...
                $this->call([RoleAsPermissionSeeder::class]);
                $this->command->info('Permission Has Role Ajouté avec succes');
            } else {
    ...
    private function createUser($role)
    {
        if ($role->name == 'Admin')
        {
            $this->call([UserTableSeeder::class]);
            $user = User::find(1);
            $this->command->info('Admin name  : '.$user->name);
        }
        else
        {
